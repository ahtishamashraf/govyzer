...
export async function createListing({ organizationId, actor, payload, request = {} }) {
  ...
  const created = await withTransaction(db, async (trx) => {
    const reference = await nextReference({ trx, organizationId, entity: 'listing', prefix: actor?.referencePrefix ?? 'GVZ' });
    const id = newId();
    const { amenity_codes: amenityCodes, version, ...rest } = payload;

    const signature = listingDuplicateSignature({
      organization_id: organizationId,
    ...
    if (amenityCodes?.length) await setAmenities({ trx, organizationId, listingId: id, codes: amenityCodes });
...
export async function updateListing({ organizationId, actor, id, payload, request = {} }) {
  const db = getDb();
  const before = await db('listings').where({ id, organization_id: organizationId }).whereNull('deleted_at').first();
  ...
  await withTransaction(db, async (trx) => {
    ...
    if (updated === 0) throw new ConflictError('This listing was changed by someone else. Reload and try again.');

    if (amenityCodes) await setAmenities({ trx, organizationId, listingId: id, codes: amenityCodes });
    ...
    const listing = await trx('listings').where('id', id).first();
    await snapshotVersion({ trx, organizationId, listing, summary: `updated: ${Object.keys(updates).join(', ')}`, actor });
  ...
  await recordAudit({ organizationId, actor, action: 'listing.updated', entityType: 'listing', entityId: id, before, after, requestId: request.requestId });
  return after;
}

export async function changeStatus({ organizationId, actor, id, status, reason }) {
  const db = getDb();
  const listing = await db('listings').where({ id, organization_id: organizationId }).whereNull('deleted_at').first();
  if (!listing) throw new NotFoundError('Listing');
  listingStateMachine.assert(listing.status, status);

  await withTransaction(db, async (trx) => {
    await trx('listings').where('id', id).update({
      status,
  ...
  await recordAudit({ organizationId, actor, action: 'listing.status_changed', entityType: 'listing', entityId: id, before: { status: listing.status }, after: { status, reason } });
...
export async function submitForApproval({ organizationId, actor, id }) {
  ...
  await withTransaction(db, async (trx) => {
    await trx('listings').where('id', id).update({ status: 'internal_review', updated_at: trx.fn.now(), updated_by: actor.membershipId });
...
export async function decideApproval({ organizationId, actor, id, decision, reason, checklist }) {
  ...
  await withTransaction(db, async (trx) => {
    await trx('listing_approvals').where('id', approval.id).update({
      status: decision,
    ...
    await trx('listings').where('id', id).update({ status: nextStatus, updated_at: trx.fn.now(), updated_by: actor.membershipId });
    await trx('listing_availability_history').insert({
      id: newId(),
      organization_id: organizationId,
      listing_id: id,
    ...
    await emitEvent(trx, {
      organizationId,
      eventType: decision === 'approved' ? EVENT_TYPES.LISTING_APPROVED : EVENT_TYPES.LISTING_REJECTED,
      aggregateType: 'listing',
      aggregateId: id,
      payload: { listing_id: id, reference: listing.reference, reason: reason ?? null, is_exclusive: Boolean(listing.is_exclusive) },
    });
  });

  await recordAudit({ organizationId, actor, action: `listing.${decision}`, entityType: 'listing', entityId: id, before: { status: listing.status }, after: { status: nextStatus, reason } });
  return db('listings').where('id', id).first();
}
...
export async function getListing({ organizationId, id }) {
  ...
  const [publications, media, agents, approvals, permits, priceHistory, availabilityHistory, amenities] = await Promise.all([
    db('portal_publications').where({ organization_id: organizationId, listing_id: id }).whereNull('deleted_at'),
    db('media_assets').where({ organization_id: organizationId, entity_type: 'listing', entity_id: id }).whereNull('deleted_at').orderBy('position'),
    ...
    db('listing_approvals').where({ organization_id: organizationId, listing_id: id }).orderBy('created_at', 'desc'),
    db('listing_permits').where({ organization_id: organizationId, listing_id: id }).whereNull('deleted_at'),
    db('listing_price_history').where({ organization_id: organizationId, listing_id: id }).orderBy('created_at', 'desc').limit(50),
    db('listing_availability_history').where({ organization_id: organizationId, listing_id: id }).orderBy('created_at', 'desc').limit(50),
    db('entity_amenities')
  ...
  return {
    ...
    publications: publications.map((publication) => ({
      ...publication,
      validation_errors: typeof publication.validation_errors === 'string' ? JSON.parse(publication.validation_errors ?? '[]') : publication.validation_errors,
    })),
    media,
    agents,
    approvals,
